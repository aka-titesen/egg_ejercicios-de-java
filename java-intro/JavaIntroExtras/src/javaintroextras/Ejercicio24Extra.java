/*
 CONSIGNA:
24. Realizar un programa que complete un vector con los N primeros números de la sucesión de Fibonacci. 
Recordar que la sucesión de Fibonacci es la sucesión de los siguientes números: 
1, 1, 2, 3, 5, 8, 13, 21, 34, ... Donde cada uno de los números se calcula sumando los dos anteriores a él.
Por ejemplo: La sucesión del número 2 se calcula sumando (1+1) Análogamente, la sucesión del número 3 es (1+2),
Y la del 5 es (2+3), Y así sucesivamente... La sucesión de Fibonacci se puede formalizar de acuerdo a la siguiente fórmula: 
Fibonaccin = Fibonaccin-1 + Fibonaccin-2 para todo n>1 Fibonaccin = 1 para todo n<=1 
Por lo tanto, si queremos calcular el término “n” debemos escribir una función que reciba como parámetro el valor de “n” 
y que calcule la serie hasta llegar a ese valor. 
Para conocer más acerca de la serie de Fibonacci consultar el siguiente link: https://quantdare.com/numeros-de-fibonacci/ 
 */
package javaintroextras;

public class Ejercicio24Extra {

    public static void main(String[] args) {
        int resultadoRecursivo = fibonacciRecursivo(10);
        int resultadoIterativo = fibonacciIterativo(10);
        System.out.println("Recursivo: " + resultadoRecursivo);
        System.out.println("Iterativo: " + resultadoIterativo);
    }

    public static int fibonacciRecursivo(int numero) {
        if (numero == 0 || numero == 1) {
            return numero;
        }
        return fibonacciRecursivo(numero - 1) + fibonacciRecursivo(numero - 2);
    }

    public static int fibonacciIterativo(int numero) {
        if (numero <= 1) {
            return numero;
        }
        int previo = 0;
        int actual = 1;
        for (int i = 2; i <= numero; i++) {
            int temporal = previo + actual;
            previo = actual;
            actual = temporal;
        }
        return actual;
    }

}
